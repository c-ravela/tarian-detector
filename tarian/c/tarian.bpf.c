...
int BPF_KPROBE(tdf_execve_e, struct pt_regs *regs) {
  ...
  /*====================== PARAMETERS ======================*/
  tdf_flex_save(&te, TDT_STR, get_syscall_param(regs, 0) /* filename */, 0,
                USER);
  tdf_flex_save(&te, TDT_STR_ARR, get_syscall_param(regs, 1) /* argv */, 0,
                USER);
  tdf_flex_save(&te, TDT_STR_ARR, get_syscall_param(regs, 2) /* envp */, 0,
                USER);
  /*====================== PARAMETERS ======================*/
...
int BPF_KPROBE(tdf_execveat_e, struct pt_regs *regs) {
  tarian_event_t te;
  int resp =
      new_event(ctx, TDE_SYSCALL_EXECVEAT_E, &te, VARIABLE, TDS_EXECVEAT_E);
  if (resp != TDC_SUCCESS) {
    stats__add(resp);
    return resp;
  }
  ...
  tdf_flex_save(&te, TDT_STR, get_syscall_param(regs, 1) /* filename */, 0,
                USER);
  tdf_flex_save(&te, TDT_STR_ARR, get_syscall_param(regs, 2) /* argv */, 0,
                USER);
  tdf_flex_save(&te, TDT_STR_ARR, get_syscall_param(regs, 3) /* envp */, 0,
                USER);
...
int BPF_KPROBE(tdf_clone_e, struct pt_regs *regs) {
  ...
  int parent_tid;
  bpf_probe_read_user_str(&parent_tid, sizeof(parent_tid),
                          (void *)get_syscall_param(regs, 2));
  tdf_save(&te, TDT_S32, &parent_tid /* parent_tidptr */);

  int child_tid;
  bpf_probe_read_user_str(&child_tid, sizeof(child_tid),
                          (void *)get_syscall_param(regs, 3));
...
int BPF_KPROBE(tdf_openat2_e, struct pt_regs *regs) {
  tarian_event_t te;
  int resp =
      new_event(ctx, TDE_SYSCALL_OPENAT2_E, &te, VARIABLE, TDS_OPENAT2_E);
  if (resp != TDC_SUCCESS) {
    stats__add(resp);
    return resp;
  }
  ...
  bpf_probe_read_user((void *)&how, bpf_core_type_size(struct open_how),
                      (void *)get_syscall_param(regs, 2));
...
int BPF_KPROBE(tdf_accept_e, struct pt_regs *regs) {
  ...
  int addrlen;
  bpf_probe_read_user(&addrlen, sizeof(addrlen),
                      (void *)get_syscall_param(regs, 2));
...
int BPF_KPROBE(tdf_connect_e, struct pt_regs *regs) {
  tarian_event_t te;
  int resp =
      new_event(ctx, TDE_SYSCALL_CONNECT_E, &te, VARIABLE, TDS_CONNECT_E);
  if (resp != TDC_SUCCESS) {
    stats__add(resp);
    return resp;
  }
