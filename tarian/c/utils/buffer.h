...
stain int save_to_buffer(program_data_t *pd, void *src, u8 type, u8 index) {
  ...
  case STR_ARR_T: {
    // save_str_arr(&pd->event->buf, (int *)&pd->cursor, (const char *const
    // *)src, type, index);
    break;
  }
  }
...
stain int save_str(syscall_buffer_t *sb, u32 *cursor, void *src, u8 type,
                   u8 index) {
  /*
  ...
  */
  ...
  int sz =
      bpf_probe_read_str(&(sb->data[((int)(*cursor) + 4) & (SYS_BUF_SIZE - 1)]),
                         MAX_STRING_SIZE, src);
  if (sz > 0) {
    barrier();
    ...
    bpf_probe_read(&(sb->data[((int)(*cursor) + 1) & (SYS_BUF_SIZE - 1)]), 1,
                   &type);
    bpf_probe_read(&(sb->data[((int)(*cursor) + 2) & (SYS_BUF_SIZE - 1)]), 2,
                   &sz);
...
stain int save_number(syscall_buffer_t *sb, u32 *cursor, void *src, u8 type,
                      u8 index, u8 size) {
  /*
  ...
  */
  ...
  if (bpf_probe_read(&(sb->data[((int)(*cursor) + 2) & (SYS_BUF_SIZE - 1)]),
                     size, src) == 0) {
    bpf_probe_read(&(sb->data[((int)(*cursor)) & (SYS_BUF_SIZE - 1)]), 1,
                   &index);
    bpf_probe_read(&(sb->data[((int)(*cursor) + 1) & (SYS_BUF_SIZE - 1)]), 1,
                   &type);
...
stain int save_str_arr(syscall_buffer_t *sb, int *cursor,
                       const char *const *src, u8 type, u8 index) {
  /*
...
#pragma unroll
  for (int i = 0; i < MAX_STR_ARR_ELEM; i++) {
    ...
    sz = bpf_probe_read_str(
        &(sb->data[((*cursor + sizeof(u16)) & (SYS_BUF_SIZE - 1))]),
    ...
    if (sz > 0) {
      ...
      bpf_probe_read(&(sb->data[(*cursor & (SYS_BUF_SIZE - 1))]), sizeof(u16),
                     &sz);
      if (sz < 0 || sz > 10000) {
        goto exit;
      }
...
// stain int save_str_arr(syscall_buffer_t *sb, u32 *cursor, const char *const
// *src, u8 type, u8 index) {
//   /*
//       Data save format: [index 1B][type 1B][string count 1B][str1 size
//      2B][str1][str2 size][str2].....
//   */
...
//     int sz = bpf_probe_read_str(&(sb->data[(((int)(*cursor)) + 2)]),
//     MAX_STRING_SIZE, arg); if (sz > 0) {
//       if (*cursor > SYS_BUF_SIZE - MAX_STRING_SIZE - sizeof(u16))
//         goto exit;
...
//   int sz = bpf_probe_read_str(&(sb->data[((int)(*cursor)) & (SYS_BUF_SIZE -
//   1)]), MAX_STRING_SIZE, ellipsis); if (sz > 0) {
//     if (*cursor > SYS_BUF_SIZE - 2)
//       goto exit;

//     // bpf_probe_read(&(sb->data[((int)(*cursor)) & (SYS_BUF_SIZE - 1)]),
//     sizeof(u16), &sz); *cursor += sz + sizeof(u16); elem_count++;
//   }
