...
stain int new_event(void *ctx, int tarian_event, tarian_event_t *te,
                    enum allocation_type at, int req_buf_sz) {
  stats__add_trigger();
  ...
  if (!ss)
    return TDCE_SCRATCH_SPACE_ALLOCATION;
  ...
  bpf_probe_read_kernel_str(te->tarian->meta_data.task.cwd,
                            len & (MAX_TARIAN_PATH - 1), filepath);
