...
#define LINUX_VERSION_CODE                                                     \
  KERNEL_VERSION(LINUX_VERSION_MAJOR, LINUX_VERSION_MINOR, LINUX_VERSION_PATCH)
#endif
...
#define MD_SIZE                                                                \
  sizeof(tarian_meta_data_t) /* sizeof tarian meta data for each event*/
...
#define TDS_EXECVEAT_E                                                         \
  (MD_SIZE + sizeof(int32_t) * 2 + MAX_STRING_SIZE * 2 + PARAM_SIZE * 2)
...
#define TDS_READ_E                                                             \
  (MD_SIZE + sizeof(int32_t) + MAX_STRING_SIZE + PARAM_SIZE + sizeof(uint32_t))
...
#define TDS_OPEN_E                                                             \
  (MD_SIZE + MAX_STRING_SIZE + PARAM_SIZE + sizeof(int32_t) + sizeof(uint32_t))
#define TDS_OPEN_R (MD_SIZE + sizeof(int32_t))

#define TDS_READV_E                                                            \
  (MD_SIZE + sizeof(int32_t) * 2 + MAX_STRING_SIZE + PARAM_SIZE)
...
#define TDS_ACCEPT_E                                                           \
  (MD_SIZE + sizeof(int32_t) * 2 + MAX_UNIX_SOCKET_PATH + PARAM_SIZE)
