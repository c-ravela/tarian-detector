...
stain int16_t write_str(uint8_t *, uint64_t *, unsigned long, uint16_t,
                        enum memory);
stain int16_t write_byte_arr(uint8_t *, uint64_t *, unsigned long, uint16_t,
                             enum memory);
...
stain int16_t write_str(uint8_t *buf, uint64_t *pos, unsigned long data_ptr,
...
stain int write_str_arr(uint8_t *buf, uint64_t *pos, u64 reserved_space,
                        char **data_ptr, uint16_t n) {
  /*
    [len..str....]
  */
...
#pragma unroll
  for (; n < MAX_CHARBUF_POINTERS; ++n) {
    bpf_probe_read_user(&charbuf_pointer, sizeof(charbuf_pointer),
                        &data_ptr[n]);
    if (!charbuf_pointer)
      break;
    ...
    arg_len = bpf_probe_read_user_str(&buf[SAFE_ACCESS(*pos)], MAX_STRING_SIZE,
...
stain int16_t write_byte_arr(uint8_t *buf, uint64_t *pos,
  ...
  *pos += sizeof(uint16_t);
  ...
  if (mr == USER) {
    written_bytes =
        bpf_probe_read_user_str(&buf[SAFE_ACCESS(*pos)], n, (void *)data_ptr);
  } else {
    written_bytes =
        bpf_probe_read_kernel_str(&buf[SAFE_ACCESS(*pos)], n, (void *)data_ptr);
  }
...
stain void write_iovec_arr(uint8_t *buf, uint64_t *pos, unsigned long iov_ptr,
                           unsigned long iov_count) {
  /*
    [[len]...str...]
  */
  ...
  *pos += sizeof(uint16_t);
  ...
  if (bpf_probe_read_user((void *)&buf[MAX_PARAM_SIZE],
                          SAFE_ACCESS(total_iovec_size), (void *)iov_ptr) != 0)
    return;

  const struct iovec *iovs = (const struct iovec *)&buf[MAX_PARAM_SIZE];

  for (int i = 0; i < MAX_IOVEC_COUNT; i++) {
    if (i == (iov_count & (MAX_IOVEC_COUNT - 1)))
      break;

    uint16_t byte_read =
        bpf_probe_read_user(&buf[SAFE_ACCESS(*pos)],
                            SAFE_ACCESS(iovs[i].iov_len), iovs[i].iov_base);
    if (byte_read != 0)
      continue;
...
stain void write_sockaddr(uint8_t *buf, uint64_t *pos, unsigned long data_ptr,
                          uint16_t addrlen) {
  if (bpf_probe_read((void *)&buf[MAX_PARAM_SIZE], SAFE_ACCESS(addrlen),
                     (void *)data_ptr) != 0)
    return;
